 there SCreenshots\# SciTool
This is a VBA library of basic algorithms commonly used in data analysis. Althouhg there are many state of the art implementations of these algorithms in other languages like Matlab, R or Python, they are not often seen in VBA. While there are good reasons to not use Excel or VBA for these type of analysis, but if you are stuck with Excel either because of budget or IT policy constraints, then hopefully this library can offer a bit of help.
The library is partly written for myself as a learning process, which is why there are some redundant alogirhtms that are not really necessary and can be replaced by native functions of Excel. I jsut wrote them for the heck of learning it. This is still an ongoing project and better documentations will come in time.
...
In the figures above, the samples with black circles around them are discovered as the exemplars, and the lines show the samples assigned to each of these exemplars. The 2D mapping was done with PCA, notice how each exemplar tendsto sit at the center its group.
